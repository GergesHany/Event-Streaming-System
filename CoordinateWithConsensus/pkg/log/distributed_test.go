...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		dataDir, err := ioutil.TempDir("", fmt.Sprintf("distributed-log-test-%d", i))
	...
	for _, record := range records {
		...
		require.Eventually(t, func() bool {
			for j := 1; j < nodeCount; j++ {
				r, err := logs[j].Read(off)
				if err != nil {
					return false
				}
				if r.Value == nil || string(r.Value) != string(record.Value) {
					return false
				}
			}
			return true
	...
	// this node should not have the new record because it left the cluster
